...
import { X, Play, Pause, SkipBack, SkipForward, ArrowRight, CornerDownRight, Zap, Code, ShieldCheck, Database, Server, Smartphone, Layout, RefreshCw } from 'lucide-react';
...
export const SequenceDiagramModal: React.FC<SequenceDiagramModalProps> = ({
  ...
  const activeHop = hops[activeStep] || hops[0];
  ...
  return (
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/85 backdrop-blur-md p-4 animate-fade-in">
      <div
        className={`w-full max-w-6xl h-[90vh] rounded-3xl border flex flex-col shadow-2xl overflow-hidden font-sans ${
      ...
      >
        ...
        <div className="p-6 border-b border-slate-800/80 flex items-center justify-between bg-slate-950/40 shrink-0">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-pink-500 via-purple-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-pink-500/30">
              <Zap className="w-5 h-5 text-white" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="text-lg font-bold font-mono text-white">Full-Stack Request/Response Sequence Flow</h2>
        ...
        <div className="flex-1 overflow-auto p-6 flex flex-col custom-scrollbar">
          {/* Top Swimlane Headers */}
          <div className="grid grid-cols-7 gap-3 pb-6 border-b border-slate-800/80 sticky top-0 bg-[#090d16]/95 z-20 backdrop-blur-md">
            {LANES.map((lane) => {
              const Icon = lane.icon;
              return (
                <div
                  key={lane.id}
                  className="p-3 rounded-2xl border flex flex-col items-center text-center shadow-lg transition-all"
                  style={{
                    backgroundColor: `${lane.bg}66`,
                    borderColor: `${lane.color}44`,
                  }}
                >
                  <div
                  ...
                  <span className="text-xs font-bold font-mono text-white truncate max-w-full">{lane.label}</span>
                  <span className="text-[10px] font-mono text-slate-400 opacity-80">{lane.sub}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="relative flex-1 py-8 space-y-8 min-w-[700px]">
            {/* Vertical Lifelines */}
            <div className="absolute inset-0 grid grid-cols-7 gap-3 pointer-events-none z-0">
              {LANES.map((lane) => (
                <div key={lane.id} className="flex justify-center h-full">
                  <div className="w-0.5 h-full bg-slate-800/80 border-dashed border-l border-slate-700/50" />
                </div>
              ))}
            </div>
            ...
            {hops.map((hop, idx) => {
              ...
              return (
                <div
                  ...
                  className={`relative z-10 cursor-pointer p-4 rounded-2xl border transition-all ${
                    isSelected
                      ? 'bg-slate-900/90 border-pink-500/80 shadow-2xl ring-2 ring-pink-500/40'
                      : isPassed
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-mono text-xs font-bold text-white">{hop.title}</span>
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-indigo-500/10 text-indigo-300 border border-indigo-500/30">
                        {hop.protocol}
                      </span>
                    </div>
                    <span className="text-[11px] font-mono text-slate-400">{hop.durationMs}ms</span>
                  </div>

                  {/* Visual Lane-to-Lane Connector Arrow */}
                  <div className="grid grid-cols-7 gap-3 my-3 py-1.5 items-center">
                    <div
                      style={{
                  ...
                  {isSelected && (
                    <div className="mt-3 pt-3 border-t border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-950/60 p-3 rounded-xl">
                      <div>
